...
def plot_listen_examples(path_audio,
    ...
    S = librosa.feature.melspectrogram(y=y,sr=sr, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels, power=power,
                                       fmin=f_min, fmax=f_max, window='hamming')
    ...
    Sxx, tn, fn, ext = sound.spectrogram(y, sr, window='hann', nperseg=1024, noverlap=512)
    
    
    fig, ax = plt.subplots(figsize=(17,7))
    util.plot_spectrogram(Sxx, extent=ext, ax=ax, colorbar=False)
    plt.show()
    
    rois.find_rois_cwt(y, sr,  flims=(50,4000), tlen=2, th=0, display=True)
    plt.show()
